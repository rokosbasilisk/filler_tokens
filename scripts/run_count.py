...
from src.utils import get_optimizer, dump_dataset_to_csv, pprint_model_predictions
from src.utils import MultiLabelCausalTransformer  # Adjust import based on your utils.py
from transformers import AutoModelForSequenceClassification, AutoTokenizer  # Adjust based on your model
...
def main():
    ...
    parser.add_argument('--early_stop', action='store_true', help='Enable early stopping based on MAE')
    ...
    # Debug: Print target
    print(f"Target: {args.target}, Type: {type(args.target)}")
    ...
    GenerateMatchCountDataset(
        ...
        cot_to_string=STRING_FUNCTION_MAPPING['basic'],  # or 'fillers' based on your preference
        data_path='./data/'
    )
    ...
    base_model = AutoModelForSequenceClassification.from_pretrained(args.model_name, num_labels=1)
    model = MultiLabelCausalTransformer(base_model, input_dim=train_dataset.vocab_size, output_dim=1)
    model.to("cuda")
    ...
    loss_func = torch.nn.MSELoss()  # For regression tasks
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc="Training")):
            ...
            labels = batch['labels'].to("cuda").unsqueeze(1)  # Make it (batch_size, 1)
            ...
            if (batch_idx + 1) % 100 == 0:
                ...
                print(f"Batch {batch_idx+1}: Train Loss={loss.item():.4f}, Train MAE={mae:.4f}")
        ...
        print(f"Epoch {epoch+1} Summary: Loss={avg_epoch_loss:.4f}, MAE={avg_epoch_mae:.4f}")
        ...
        # Early Stopping
        if args.early_stop and avg_epoch_mae < 0.01:
            print("Early stopping triggered due to low MAE.")
            break
